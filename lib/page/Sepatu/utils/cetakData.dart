...
class Cetakdata {
  void getPDF(String IdCollection) async {
    try {
      final pdf = pw.Document();

      // Ambil data dari Firebase
      final snapshot =
          await FirebaseFirestore.instance.collection(IdCollection).get();
          ...
          build: (context) {
            return pw.Column(
              ...
              children: [
                ...
                pw.Center(
                  child: pw.Text(
                    'Laporan Data Sepatu $IdCollection',
                    style: pw.TextStyle(
                      fontSize: 14,
                      fontWeight: pw.FontWeight.bold,
                    ),
                ...
                pw.Table.fromTextArray(
                  ...
                  data: List<List<String>>.generate(data.length, (index) {
                    final rawHarga =
                        int.tryParse(data[index]['harga'].toString()) ?? 0;
                    return [
                      (index + 1).toString(),
                      data[index]['tipe_sepatu'] ?? '',
                      data[index]['bahan'] ?? '',
                      data[index]['berat']?.toString() ?? '',
                      data[index]['jml_size']?.toString() ?? '',
                      data[index]['jml_warna']?.toString() ?? '',
                      'Rp ${NumberFormat.decimalPattern('id_ID').format(rawHarga)}.000',
                    ];
                  }),
                ),
